...
def get_color(step, total_steps):
    intensity = int(15 + (240 - 15) * (step / total_steps))
    return f'#{intensity:02X}{intensity//2:02X}{intensity:02X}'
...
def dfs_traversal(root):
    ...
    for i, node in enumerate(visited):
        node.color = get_color(i, total - 1)
...
def main():
    ...
    tree1 = heap_to_tree(heap)
    dfs_traversal(tree1)
    
    tree2 = heap_to_tree(heap)
    bfs_traversal(tree2)
